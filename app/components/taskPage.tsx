...
export const TrainPageContent = () => {
  const [Data, setData] = useState({
    title: "",
    date: { day: 0, month: "" },
    train: [{ approach: 0, repeats: 0, weight: 0 }],
  });

  const [trains, setTrains] = useState<
    { approach: number; repeats: number; weight: number }[] | undefined
  >();
  ...
  return (
    <div className="px-[21px] pb-[100px]">
      <div className="w-full flex items-center justify-between mt-[20px]">
        <div className="flex-1 flex justify-start mt-[9px]">
          <Back />
        </div>
        <div className="flex-1 flex justify-center flex-col items-center">
          <h1 className="text-black2 font-[500] text-[28px]">{Data.title}</h1>
          <h3 className="text-black2 text-[24px]">
